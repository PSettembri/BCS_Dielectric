        ...
        if ((veclen.ge.k(ibin)).and.(veclen.lt.k(ibin+1))) then 
                ...
                epsisto(:,ibin)=epsisto(:,ibin)+totaleps(ig,:,jq)
        ...
        if (veclen.eq.k(nbin))then 
                ...
                jq=ivqiq(ivq(1,iq),ivq(2,iq),ivq(3,iq))
                epsisto(:,nbin)=epsisto(:,nbin)+totaleps(ig,:,jq)
        end if
        end do
        ...
        if (invers) then 
        open(50,file='eps_iso_lfe.dat')
        else
        open(50,file='eps_iso.dat')
        end if
        ...
        if (invers) then
        open(50,file='loss_iso_lfe.dat')
        else
        open(50,file='loss_iso.dat')
        end if
